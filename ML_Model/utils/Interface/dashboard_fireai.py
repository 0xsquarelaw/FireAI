...
def main(page: ft.Page):
    ...
    mytable = DataTable(
        ...
        column_spacing=200,
        columns=[
            DataColumn(Text("Package number")),
            DataColumn(Text("IP Source")),
            DataColumn(Text("IP Destiny")),
        ],
        rows=[]
       # bgcolor="red"
    )
    ...
    def process_packet(packet):
        ...
        if packet.haslayer(IP):
            ...
            if packet.haslayer(TCP):
                ...
                # Protocolo UDP, Origin port, destination port
                numberpack.controls.append(
            ...
            if num_al==1 :
                ...
                toast = Notification(app_id="fifthScan", 
                            title="Anomaly detection", 
                            msg="An anomaly has been detected in:", 
                            duration="short",
                            
                            )
                # Add a notification button
                toast.add_actions(label="See details")
    ...
    numberpack.controls.append(
        ft.Text("Starting packages capture...")
    )
